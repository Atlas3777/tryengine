...
public:
    ...
    std::vector<Mesh*> LoadModel(const std::string& path) {
        ...
        // Важно: aiProcess_FlipUVs нужен для OpenGL/SDL координат
        ...
        // Вычисляем папку модели, чтобы искать текстуры рядом
        // Пример: assets/models/box.gltf -> directory = assets/models/
        ...
        for (unsigned int i = 0; i < scene->mNumMeshes; i++) {
            ...
            // 3. Загружаем материал и текстуру
            aiMaterial* material = scene->mMaterials[aiMesh->mMaterialIndex];
            aiString texPath;
            // Ищем Diffuse текстуру (Base Color)
            if (material->GetTexture(aiTextureType_DIFFUSE, 0, &texPath) == AI_SUCCESS) {
                // Склеиваем путь: папка модели + имя файла из gltf
                std::string fullTexPath = directory + texPath.C_Str();
                myMesh->texture = LoadTexture(fullTexPath);
            }

            // Если текстуры нет, можно назначить дефолтную белую заглушку здесь

            // 4. Заливаем геометрию в GPU (аналогично текстуре)
            // Создаем буферы...
            ...
            // Upload через TransferBuffer (упрощенно)
            ...
            // Сохраняем меш в список
            // В реальном движке лучше добавлять в meshCache по имени
