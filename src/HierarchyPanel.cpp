...
void HierarchyPanel::ReparentEntity(entt::entity entity, entt::entity newParent, entt::registry& reg) {
    ...
    auto* hc = reg.try_get<HierarchyComponent>(entity);
    if (!hc) {
        hc = &reg.emplace<HierarchyComponent>(entity);
    }
    ...
    while (check != entt::null) {
        ...
        auto* parentHC = reg.try_get<HierarchyComponent>(check);
        if (!parentHC) break;
...
void HierarchyPanel::DestroyEntityRecursive(entt::entity entity, entt::registry& reg) {
    ...
    for (auto e : view) {
        if (view.get<HierarchyComponent>(e).parent == entity) {
